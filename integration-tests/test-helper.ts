...
export class TestRig {
  ...
  setup(
    ...
    const geminiDir = join(this.testDir, '.bluecode-cli');
      ...
      () => {
        ...
        for (const jsonStr of jsonObjects) {
          try {
            ...
            if (
              logData.attributes &&
              logData.attributes['event.name'] === `bluecode_cli.${eventName}`
            ) {
              return true;
            }
          } catch {
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'bluecode_cli.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const jsonStr of jsonObjects) {
      try {
        ...
        if (
          logData.attributes &&
          logData.attributes['event.name'] === 'bluecode_cli.tool_call'
        ) {
  ...
  readLastApiRequest(): Record<string, unknown> | null {
    ...
    for (const jsonStr of jsonObjects) {
      try {
        ...
        if (
          logData.attributes &&
          logData.attributes['event.name'] === 'bluecode_cli.api_request'
        ) {
