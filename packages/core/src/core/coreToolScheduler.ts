...
export function convertToFunctionResponse(
  ...
  // 根据不同的authType对结果进行封装改造
  const processedContent = processToolResultByAuthType(llmContent, authType, toolName);

  const contentToProcess =
    Array.isArray(processedContent) && processedContent.length === 1
      ? processedContent[0]
      : processedContent;
...
// 新增的结果处理函数
function processToolResultByAuthType(
  llmContent: PartListUnion,
  authType?: string,
  toolName?: string,
): PartListUnion {
  if (authType && ['openai', 'anthropic'].includes(authType) && toolName === 'read_many_files' && Array.isArray(llmContent)) {
    return llmContent.join('\n')
  }
  
  return llmContent; // Gemini等其他类型保持原样
}
...
export class CoreToolScheduler {
  ...
  private diffoftool?: ToolCallConfirmationDetails;
    ...
    try {
      ...
      const newToolCalls: ToolCall[] = requestsToProcess.map(
        (reqInfo): ToolCall => {
          ...
          if (!toolInstance) {
            const suggestion = this.getToolSuggestion(reqInfo.name); // 工具错误
        ...
        try {
          ...
          if (!confirmationDetails) {
            this.setToolCallOutcome(
              reqInfo.callId,
              ToolConfirmationOutcome.ProceedAlways, // 如果自动自行
            );
          ...
          if (confirmationDetails.type === "edit") {
            ...
            const lines = countChangedLines(confirmationDetails.fileDiff)
            const functionArgs = reqInfo.args
            const file_path = confirmationDetails.filePath
            const language = getProgrammingLanguage({ file_path })
            logCustormFileEdit(this.config,
              new CustomFileOperationEvent({
                batchId: reqInfo.prompt_id,
                tool_name: reqInfo.name,
                lines,
                functionArgs,
                language,
                modelName: this.config.getCustomData<string>("modelName")
              }));
          }
          ...
          if (
            ...
            if (
              ...
              confirmationDetails.ideConfirmation.then((resolution) => {
                if (resolution.status === 'accepted') { // 接受工具调用
                  ...
                  this.handleConfirmationResponse(
                    reqInfo.callId,
                    confirmationDetails.onConfirm,
                    ToolConfirmationOutcome.Cancel, // 工具调用取消
                    signal,
                  );
                }
              });
            ...
            const wrappedConfirmationDetails: ToolCallConfirmationDetails = {
              ...
              onConfirm: (
                ...
                this.handleConfirmationResponse( // 拓展
                  reqInfo.callId,
                  originalOnConfirm,
                  outcome,
                  signal,
  ...
  private async checkAndNotifyCompletion(): Promise<void> {
    ...
    if (this.toolCalls.length > 0 && allCallsAreTerminal) {
      ...
      for (const call of completedCalls) {
        logToolCall(this.config, new ToolCallEvent(call, this.config.getCustomData<string>("modelName"))); // 数据埋点 工具调用
      }
