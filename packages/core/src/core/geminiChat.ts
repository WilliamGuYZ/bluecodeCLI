...
function isValidResponse(response: GenerateContentResponse): boolean {
  ...
  // console.log('----isValidResponse content----', content)
...
export class GeminiChat {
      ...
      try {
        ...
        if (lastError) {
          ...
          if (self.history[self.history.length - 1] === userContent) {
            // Do NOT remove if it's a functionResponse (tool_result). Keeping C avoids losing
            // tool outputs when a stream fails, which would break subsequent turns like [A,B,C] + D.
  ...
  private async makeApiCallAndProcessStream(
    ...
    return this.processStreamResponse(streamResponse, userContent, prompt_id);
  }
  ...
  private async *processStreamResponse(
    ...
    let reasoningDetails: any = undefined; // 🎯 保存 reasoning_details
    ...
    for await (const chunk of streamResponse) {
      ...
      if (isValidResponse(chunk)) {
        ...
        // 🎯 提取 reasoning_details（通常在最后一个 chunk 中）
        if ((content as any)?.reasoning_details) {
          reasoningDetails = (content as any).reasoning_details;
        }
      } else {
        // Invalid chunk 错误已由 loggingContentGenerator 统一处理
        // 这里只记录 invalid chunk 日志，不重复上报 api_response
        logInvalidChunk(
          this.config,
    ...
    // 🎯 如果有 reasoning_details，添加到 modelOutput 中
    const modelOutput: Content[] =
  ...
  private recordHistory(
    ...
    for (const content of modelOutput) {
      ...
      if (
        ...
        // 🎯 合并时保留 reasoning_details（如果新 turn 有的话）
        if ((newTurn as any).reasoning_details) {
          (lastTurnInFinal as any).reasoning_details = (newTurn as any).reasoning_details;
        }
      } else {
