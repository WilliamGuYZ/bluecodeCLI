...
/**
 * Assistant message type - supports both legacy 'gemini' and new 'bluecode'.
 * New messages are written as 'bluecode', but 'gemini' is still supported for reading old sessions.
 */
export type AssistantMessageType = 'gemini' | 'bluecode';

/**
 * The type used when writing new assistant messages.
 */
export const ASSISTANT_MESSAGE_TYPE: AssistantMessageType = 'bluecode';

/**
 * Assistant message record with additional fields.
 */
export interface AssistantMessageRecord extends BaseMessageRecord {
...
/**
 * Checks if a message is from the assistant (supports both 'gemini' and 'bluecode' types).
 * This is a type guard that narrows the type to AssistantMessageRecord.
 */
export function isAssistantMessage(msg: MessageRecord): msg is AssistantMessageRecord {
  return msg.type === 'gemini' || msg.type === 'bluecode';
}
 ...
 *
 * Sessions are stored as JSON files in ~/.bluecode-cli/tmp/<project_hash>/chats/
 */
export class ChatRecordingService {
    ...
    try {
      this.updateConversation((conversation) => {
        ...
        if (isAssistantMessage(msg)) {
          // If it's a new assistant message then incorporate any queued thoughts.
  ...
  /**
   * Updates the tokens for the last message in the conversation (which should be by assistant).
   */
  recordMessageTokens(tokens: {
    ...
    try {
      this.updateConversation((conversation) => {
        ...
        if (lastMsg && isAssistantMessage(lastMsg) && !lastMsg.tokens) {
  ...
  /**
   * Adds tool calls to the last message in the conversation (which should be by assistant).
   */
  recordToolCalls(toolCalls: ToolCallRecord[]): void {
    ...
    try {
      this.updateConversation((conversation) => {
        const lastMsg = this.getLastMessage(conversation);
        // If a tool call was made, but the last message isn't from assistant, it's because assistant is
        // calling tools without starting the message with text.  So the user submits a prompt, and
        // assistant immediately calls a tool (maybe with some thinking first).  In that case, create
        // a new empty assistant message.
        // Also if there are any queued thoughts, it means this tool call(s) is from a new assistant
        // message--because it's thought some more since we last, if ever, created a new assistant
        ...
        if (
          ...
          const newMsg: MessageRecord = {
            ...this.newMessage(ASSISTANT_MESSAGE_TYPE, ''),
          ...
          // The last message is an existing assistant message that we need to update.
  ...
  /**
   * Deletes a session file by file path.
   */
  ...
  /**
   * Returns the current conversation record.
   */
  ...
  /**
   * Rewinds the conversation to a specific message, removing all messages after it.
   * @param messageId - The ID of the message to rewind to
   * @returns true if successful, false if message not found
   */
  rewindTo(messageId: string): boolean {
  ...
  /**
   * Saves a summary for the current conversation.
