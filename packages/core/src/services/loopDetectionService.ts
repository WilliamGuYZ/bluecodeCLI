...
export class LoopDetectionService {
  ...
  private async checkForLoopWithLLM(signal: AbortSignal) {
    const history = this.config.getGeminiClient().getHistory();
    const recentHistory = this.safeSliceHistory(history);
...
IMPORTANT: You must wrap your JSON response in a markdown code block like \`\`\`json ... \`\`\`. This helps separate your analysis from the structured output.

Your response must be a valid JSON object with exactly these two fields:
\`\`\`json
{
  "reasoning": "Your detailed reasoning about whether the conversation is looping without forward progress",
  "confidence": 0.95
}
\`\`\`
Where:
- "reasoning": A string explaining your analysis and why you reached your conclusion
- "confidence": A number between 0.0 and 1.0 representing your confidence that the conversation is in an unproductive state (0.0 = definitely not a loop, 1.0 = definitely a loop)

Examples to guide your analysis and response format (wrap JSON in code blocks):

Example 1 (No loop):
\`\`\`json
{
  "reasoning": "The assistant is making incremental progress by adding docstrings to different functions in the same file. Each tool call has a unique target, and the changes are cumulative and meaningful.", 
  "confidence": 0.1
}
\`\`\`

Example 2 (Detected loop):
\`\`\`json
{
  "reasoning": "The assistant is calling the same 'replace' tool with identical arguments 8 times in a row, attempting to replace the same text with the same content each time without success. This indicates a clear repetitive, non-productive state.", 
  "confidence": 0.98
}
\`\`\`
...
\`\`\`json
{
  "reasoning": "The assistant is alternating between two files but making small changes. It's unclear if this is deliberate refactoring or aimless cycling. The pattern has persisted for 7 turns.", 
  "confidence": 0.55
}
\`\`\`

Remember: Wrap your JSON response in markdown code blocks (\`\`\`json ... \`\`\`).`;
    const contents = [
      ...recentHistory,
  ...
  /**
   * Safely slices conversation history to preserve tool call pairing integrity.
   *
   * In the Gemini chat format, a tool call (functionCall) from the model is always
   * followed by a function response from the tool. These two messages form a pair
   * and should never be separated, as doing so would cause framework validation errors.
   *
   * This method attempts to retrieve the last `LLM_LOOP_CHECK_HISTORY_COUNT` messages
   * from the history. However, if the slice point falls between a tool call and its
   * corresponding function response, the method adjusts the slice point earlier by one
   * position to include both messages in the result.
   *
   * Algorithm:
   * 1. If history length <= LLM_LOOP_CHECK_HISTORY_COUNT, return all history.
   * 2. Calculate the ideal slice start index: history.length - LLM_LOOP_CHECK_HISTORY_COUNT.
   * 3. Scan messages from the slice start index to detect broken pairs:
   *    - If we encounter a function role message as the first message,
   *      its preceding tool call was cut off.
   *    - Adjust the slice start to include the tool call.
   * 4. Return the safely sliced history.
   *
   * Example:
   *   History: [user, assistant_with_toolCall, function_response, assistant, user]
   *   LLM_LOOP_CHECK_HISTORY_COUNT = 3
   *   Initial slice: [function_response, assistant, user]  <-- BROKEN PAIR
   *   Adjusted slice: [assistant_with_toolCall, function_response, user]  <-- PAIR PRESERVED
   *
   * @param history - The full conversation history array
   * @returns A sliced history array that preserves tool call pair integrity
   */
  private safeSliceHistory(history: Content[]): Content[] {
    if (history.length <= LLM_LOOP_CHECK_HISTORY_COUNT) {
      return history;
    }

    const startIndex = history.length - LLM_LOOP_CHECK_HISTORY_COUNT;

    for (let i = startIndex; i < history.length; i++) {
      const current = history[i];

      // current.role === 'function'
      if (isFunctionResponse(current)) {
