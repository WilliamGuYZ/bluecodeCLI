...
//   const resolved1 = path.resolve(p1);
//   const resolved2 = path.resolve(p2);
...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    if (!this.currentIde || !this.currentIdeDisplayName) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run BlueCode CLI in one of these supported IDEs: VS Code or VS Code forks`,
        false,
      );
      return;
    }
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. BlueCode CLI is running in a different location than the open workspace in ${currentIdeDisplayName}. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
  ...
  private getPortFromEnv(): string | undefined {
    ...
    if (!port) {
      this.setState(
        IDEConnectionStatus.Disconnected,
        'BlueCode CLI Companion extension not found. Install via /ide install and restart the CLI in a fresh terminal window.',
      );
      return undefined;
    }
    return port;
  ...
  //   const ideWorkspacePath = process.env['BlueCode_CLI_IDE_WORKSPACE_PATH'];
  //   if (!ideWorkspacePath) {
  //     this.setState(
  //       IDEConnectionStatus.Disconnected,
  ...
  //   const samePath = isSamePath(ideWorkspacePath, process.cwd());
  //   if (!samePath) {
  //     this.setState(
  //       IDEConnectionStatus.Disconnected,
  //       `BlueCode CLI is running in a different directory (${process.cwd()}) from the IDE's open workspace (${ideWorkspacePath}). Please run BlueCode CLI in the same directory.`,
  //     );
  //     return false;
  ...
  > {
    ...
    try {
      const portFile = path.join(
        os.tmpdir(),
        `bluecode-server-${this.ideProcessInfo.pid}.json`,
      );
