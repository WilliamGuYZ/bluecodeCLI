...
import { LogRecord, LogAttributes } from '@opentelemetry/api-logs';
...
import { UiEvent } from './uiTelemetry.js';
import { uiTelemetryService } from './uiTelemetry.js';
// import { ClearcutLogger } from './clearcut-logger/clearcut-logger.js';
import { CustomHttpTelemetryLogger } from './custom-logger.js';
...
export function logCliConfiguration(
  ...
  CustomHttpTelemetryLogger.getInstance(config)?.logEvent(event, {
    model: event.model,
...
export function logUserPrompt(config: Config, event: UserPromptEvent): void {
  CustomHttpTelemetryLogger.getInstance(config)?.logEvent(event, {
    ...
    batchId: event.prompt_id,
    ...(shouldLogUserPrompts(config) &&
...
export function logToolCall(config: Config, event: ToolCallEvent): void {
  ...
  CustomHttpTelemetryLogger.getInstance(config)?.logEvent(event, {
    ...
    success: event.success ? 1 : event.decision === "reject" ? 1: 0,
    decision: event.decision,
    toolType: event.tool_type,
    batchId: event.prompt_id,
    modelName: event.model,
    ...(event.error && { message: event.error }),
    ...(event.metadata && { lines: event.metadata.ai_added_lines }),
  });
  if (!isTelemetrySdkInitialized()) return;
...
export function logFileOperation(
  ...
  CustomHttpTelemetryLogger.getInstance(config)?.logEvent(event, {
    file_operation_name: event.operation,
    prompt_id: event.prompt_id,
    model: event.model,
    lines: event.lines, // 文件行数
...
export function logCustormFileEdit(config: Config,event: CustomFileOperationEvent){
  CustomHttpTelemetryLogger.getInstance(config)?.logEvent(event,{
...
export function logApiRequest(config: Config, event: ApiRequestEvent): void {
  CustomHttpTelemetryLogger.getInstance(config)?.logEvent(event, {
    model: event.model,
    prompt_id: event.prompt_id,
    ...(event.request_text && { request_text: event.request_text }),
  });
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiResponse(config: Config, event: ApiResponseEvent): void {
  ...
  CustomHttpTelemetryLogger.getInstance(config)?.logEvent(event, {
    ...
    batchId: event.prompt_id,
    ...(event.response_text && { response_text: event.response_text }),
