...
**Output Format:**
Respond *only* in JSON format according to the following schema. Do not include any text outside the JSON structure. Do not wrap the JSON in code fences or markdown. Return only the JSON object.
\`\`\`json
{
  "type": "object",
  "properties": {
...
\`\`\`
If your answer cannot be expressed as a valid JSON object exactly matching the schema, respond with an empty JSON object: {}
`;
...
export async function checkNextSpeaker(
  ...
  try {
    // Use the same configured model as streaming paths, falling back handled inside generateJson
    const configuredModel = geminiClient.getModel?.()
    ...
    console.warn(
      'Failed to talk to BlueCode endpoint when seeing if conversation should continue.',
      error,
    );
    return null;
  }
