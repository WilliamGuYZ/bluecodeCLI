...
export class McpClient {
  ...
  constructor(
    ...
    this.client = new Client({
      name: `bluecode-cli-mcp-client-${this.serverName}`,
      version: '0.0.1',
    });
  }
 ...
 * 查找所有的mcp
 * @param mcpServers A record of named MCP server configurations. settings.json注册的mcp
 * @param mcpServerCommand An optional command string for a dynamically specified MCP server.
 * @param toolRegistry The central registry where discovered tools will be registered. 工具注册对象
  ...
  try {
    ...
    const discoveryPromises = Object.entries(mcpServers).map(
      ([mcpServerName, mcpServerConfig]) =>
        connectAndDiscover( // mcp连接测试 测试成功注册到工具
          mcpServerName,
          mcpServerConfig,
          toolRegistry,
          promptRegistry,
