...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'bluecode-cli-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `bluecode-cli-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `bluecode-cli-modify-${fileName}-new-${timestamp}${ext}`,
  );
