...
import { isGitRepository } from '@vivo/bluecode-cli-core';

vi.mock('@vivo/bluecode-cli-core', () => ({
  isGitRepository: vi.fn(),
}));

vi.mock('../config/hosts.js', () => {
  return {
    getHost: vi.fn(async () => ({
      host: 'http://aicode-api-test.vmic.xyz',
...
describe('getInstallationInfo', () => {
  ...
  it('should detect Homebrew installation via execSync', async () => {
    ...
    mockedExecSync.mockReturnValue(Buffer.from('bluecode-cli')); // Simulate successful command

    const info = await getInstallationInfo(projectRoot, false);

    expect(mockedExecSync).toHaveBeenCalledWith(
      'brew list -1 | grep -q "^bluecode-cli$"',
      { stdio: 'ignore' },
    );
  ...
  it('should detect global pnpm installation', async () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@vivo/bluecode-cli/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toContain('pnpm add -g @vivo/bluecode-cli@latest --registry=');
  ...
  it('should detect global yarn installation', async () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@vivo/bluecode-cli/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toContain('yarn global add @vivo/bluecode-cli@latest --registry=');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', async () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toContain('bun add -g @vivo/bluecode-cli@latest --registry=');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should default to global npm installation for unrecognized paths', async () => {
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toContain('npm install -g @vivo/bluecode-cli@latest --registry=');
    expect(info.updateMessage).toContain('Attempting to automatically update');
