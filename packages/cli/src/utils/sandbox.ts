...
import type { Config, SandboxConfig } from '@vivo/bluecode-cli-core';
import { FatalSandboxError } from '@vivo/bluecode-cli-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'bluecode-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'bluecode-cli-sandbox';
const SANDBOX_PROXY_NAME = 'bluecode-cli-sandbox-proxy';
...
export async function start_sandbox(
  ...
  try {
    ...
    // determine full path for bluecode-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under bluecode-cli repo
    //
    // note this can only be done with binary linked from bluecode-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('bluecode-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed gemini binary; ' +
            'run `npm link ./packages/cli` under bluecode-cli repo to switch to linked binary.',
        );
      } else {
    ...
    if (!(await ensureSandboxImageIsPresent(config.command, image))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the bluecode-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify bluecode-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
