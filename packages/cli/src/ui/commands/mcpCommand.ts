...
import type { DiscoveredMCPPrompt } from '@vivo/bluecode-cli-core';
import {
  DiscoveredMCPTool,
...
const authCommand: SlashCommand = {
  name: 'auth',
  description: '使用支持 OAuth 的 MCP 服务器进行身份验证',
    ...
    if (!serverName) {
      ...
      if (oauthServers.length === 0) {
        return {
          type: 'message',
          messageType: 'info',
          content: '没有配置 OAuth 身份验证的 MCP 服务器',
        };
      }
