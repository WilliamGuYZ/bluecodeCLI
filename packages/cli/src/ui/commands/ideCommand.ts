...
import type { Config, IdeClient, File } from '@vivo/bluecode-cli-core';
import {
  getIdeInstaller,
  IDEConnectionStatus,
  ideContext,
...
function getIdeStatusMessage(ideClient: IdeClient): {
  ...
  switch (connection.status) {
    case IDEConnectionStatus.Connected:
      return {
        messageType: 'info',
        content: `🟢 已连接到 ${ideClient.getDetectedIdeDisplayName()}`,
      };
...
export const ideCommand = (config: Config | null): SlashCommand | null => {
    ...
    return {
      ...
      action: (): SlashCommandActionReturn =>
        ({
          type: 'message',
          messageType: 'error',
          content: `您当前的环境不支持 IDE 集成。要使用此功能，请在以下受支持的 IDE 之一中运行 BlueCode CLI：VS Code 或 VS Code forks。`,
        }) as const,
    };
  }
  ...
  const installCommand: SlashCommand = {
    name: 'install',
    description: `为 ${ideClient.getDetectedIdeDisplayName()} 安装所需的 IDE 配套插件`,
    kind: CommandKind.BUILT_IN,
