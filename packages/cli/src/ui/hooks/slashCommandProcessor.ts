...
export const useSlashCommandProcessor = (
  ...
  getConsoleMessages: () => ConsoleMessageItem[],
) => {
  ...
  const handleSlashCommand = useCallback(
    async (
      ...
      // Check if the first token after '/' contains path separators,
      // which likely indicates a file path rather than a command.
      const firstTokenMatch = trimmed.substring(1).trim().match(/^(\S+)/);
      if (firstTokenMatch) {
        const firstToken = firstTokenMatch[1];
        // If the first token contains path separators, treat it as a regular prompt
      ...
      try {
                ...
                case 'dialog':
                  switch (result.dialog) {
                    ...
                    case 'privacy':
                      ...
                      return { type: 'handled' };
                    case 'model':
                      // 当用户输入/model命令时，调用openModelDialog函数
                      // 这个函数来自App.tsx，会设置isModelDialogOpen状态为true
                      openModelDialog();
                      return { type: 'handled' }; // 告诉系统命令已被处理
                    case 'settings':
                      openSettingsDialog();
                      return { type: 'handled' };
                    case 'sessionBrowser':
        ...
        // If no command was found, treat it as a regular prompt instead of showing an error
        // This handles cases like "/api/v1/review 为我讲解这个接口的具体实现"
