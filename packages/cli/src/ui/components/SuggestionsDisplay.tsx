...
export function SuggestionsDisplay({
  ...
  if (isSlashCommandMode) {
    ...
    commandNameWidth = Math.max(
      15,
      Math.min(maxLabelLength + 12, maxAllowedWidth),
    );
  }
  if (visibleSuggestions.length > 0 && visibleSuggestions[0].label && !visibleSuggestions[0].description){
    // 在chat delete的时候，description为空 label占满，减少换行
    commandNameWidth = width - 4;
  }
  return (
