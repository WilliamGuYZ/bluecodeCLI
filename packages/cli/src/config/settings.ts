...
export const SETTINGS_DIRECTORY_NAME = '.bluecodeCli';
...
const MIGRATION_MAP: Record<string, string> = {
  ...
  sessionRetention: 'general.sessionRetention',
  theme: 'ui.theme',
...
export function getSystemSettingsPath(): string {
  ...
  if (platform() === 'darwin') {
    return '/Library/Application Support/bluecodeCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\bluecode-cli\\settings.json';
  } else {
    return '/etc/bluecode-cli/settings.json';
  }
}
...
export function loadEnvironment(settings?: Settings): void {
  ...
  // console.log('----loadEnvironment---- found env file:', envFilePath);
  ...
  if (envFilePath) {
    ...
    // console.log('----loadEnvironment---- loaded env from:', envFilePath);
    // console.log('----loadEnvironment---- CUSTOM_LLM_BASE_URL:', process.env.CUSTOM_LLM_BASE_URL);
...
/**
 * Default session retention configuration.
 * Applied on first launch when user hasn't configured sessionRetention.
 */
...
/**
 * Ensures default sessionRetention config is written to user settings on first launch.
 * Only writes if user hasn't explicitly configured sessionRetention (including setting enabled: false).
 *
 * @param settings - The loaded settings object
 * @returns true if default config was written, false otherwise
 */
export function ensureDefaultSessionRetention(settings: LoadedSettings): boolean {
  const userSettings = settings.user.settings;

  // Check if sessionRetention is already configured in user settings
  // We check the raw user settings file, not merged, to avoid overwriting explicit user config
  const hasSessionRetention =
    userSettings.general?.sessionRetention !== undefined ||
    // Also check V1 format (flat structure) for backwards compatibility
    (userSettings as Record<string, unknown>)['sessionRetention'] !== undefined;

  if (hasSessionRetention) {
    return false;
  }

  // Write default sessionRetention to user settings
  settings.setValue(
    SettingScope.User,
