...
if (!baseImage?.length) {
  console.warn(
    'No default image tag specified in bluecode-cli/packages/cli/package.json',
  );
}
...
console.log('packing @vivo/bluecode-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'google-bluecode-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @vivo/bluecode-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @vivo/bluecode-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'google-bluecode-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @vivo/bluecode-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `google-bluecode-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `google-bluecode-cli-core-${packageVersion}.tgz`),
  0o755,
);
