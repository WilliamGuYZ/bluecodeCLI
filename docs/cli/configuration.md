...
- **`--output-format <format>`**:
  - Sets the output format for non-interactive mode. Available formats:
    - `text` (default): Regular human-readable text output
    - `json`: Structured JSON output with response, stats, and error information
    - `stream-json`: Real-time event-streaming JSON (newline-delimited) for monitoring and pipelines
  - Only applies when using `--prompt` or piping input from stdin
  - Example: `gemini --prompt "Explain Docker" --output-format json`
- **`--version`**:
