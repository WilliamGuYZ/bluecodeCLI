...
You can control the output format in non-interactive mode using the `--output-format` flag:
...
- **JSON format:**
  ...
  ```
  Returns structured JSON with `response`, `stats` (turns, tool calls), and optional `error` fields.

- **Stream JSON format:**
  ```bash
  gemini -p "What is fine tuning?" --output-format stream-json
  ```
  Returns newline-delimited JSON events in real-time, useful for monitoring and pipelines.
